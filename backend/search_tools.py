...
class CourseSearchTool(Tool):
    ...
    def __init__(self, vector_store: VectorStore):
        ...
        self.last_sources: List[Dict[str, Any]] = []  # Track sources from last search
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        formatted = []
        sources: List[Dict[str, Any]] = []  # Track sources for the UI
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            if source_key not in seen_sources:
                ...
                # Get lesson URL from vector store
                url = None
                if lesson_num is not None:
...
class CourseOutlineTool(Tool):
    """Tool for retrieving course outlines including lesson list"""
    ...
    def _format_outline(self, metadata: Dict[str, Any]) -> str:
        lines = []
        lines.append(f"Course: {metadata.get('title', 'Unknown')}")

        if metadata.get("course_link"):
            lines.append(f"Course Link: {metadata['course_link']}")

        lines.append(f"\nLessons ({metadata.get('lesson_count', 0)} total):")
